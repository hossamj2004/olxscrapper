...
foreach($filterGroups as $filterGroup ){
	...
	foreach($filterGroup->filterList() as $filter ) {
		...
		echo '<form method="post">'.
			'<input type="hidden" name="filter_group_id" value="'.$filter->id.'" > 
...
foreach($filterGroups as $filterGroup ){
	...
	$condition .= " 1=1 ";
	foreach($filterGroup->filterList() as $filter ) {
		//$condition.= " AND  ".$filter->filter_field." ".$filter->operator." ".$filter->filter_value." ";
	}
	$condition .= " ) ";
}
...
//run query
$ads = $db->ad();
$ads = $ads->leftJoin( "attribute" ) ;
$ads = $ads->where($condition);
$ads = $ads->orderBy( 'ad.id', 'DESC' )->limit(10);
?>
