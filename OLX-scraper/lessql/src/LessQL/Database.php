...
class Database {
	...
	function select( $table, $options = array() ) {
		...
		//$query .=', '. $this->quoteIdentifier( $table ).".id"." as id  ";
		...
		$options[ 'groupBy' ] = $this->quoteIdentifier( $table ).'.id ';
		$query .= $this->getSuffix( $options[ 'where' ], $options[ 'orderBy' ], $options[ 'limitCount' ], $options[ 'limitOffset' ], $options[ 'groupBy' ] );
...
if ( !empty( $options[ 'join' ] )  ) {
	 ...
	 * @return string
	 */
	function getSuffix( $where, $orderBy = array(), $limitCount = null, $limitOffset = null ,$groupBy = null ) {
