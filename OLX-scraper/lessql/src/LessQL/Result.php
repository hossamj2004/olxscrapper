...
class Result implements \IteratorAggregate, \JsonSerializable {
	...
	public $joinCode='';
	public function leftJoin($params=null){
		$clone = clone $this;
		$clone->joinCode .= ' left join '.$params.' on '.$this->table.'.id = '.$this->table.'_id ';
		return $clone;
	}
	...
	public function search($params=null)
	{
		...
		if(count($aModelsToJoin) > 0) {
			...
			foreach ($aModelsToJoin as $model) {
				$clone->leftJoin(rtrim($model[0],'.'));
			}
		}

		if(isset($params['conditions'])) {
			$clone->where($params['conditions'],$params['bind']);
		}

		if(isset($params['order'])) {
			$clone->order($params['order']);
		}
